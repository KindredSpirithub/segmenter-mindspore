...
def parse_args():
    ...
    parser.add_argument("--pth_path", default = "./deit_base_distilled_patch16_384.ckpt", type=str, help="Torch model path.")
    parser.add_argument("--ckpt_path", default="./deit_base_distilled_patch16_384_ms.ckpt", type=str, help="Output checkpoint storage path.")
...
def convert(pth_path, ckpt_path):
    ...
    while i < len(keys):
        ...
        if "weight" in temp and "norm" in temp:
                temp = temp.replace("weight", "gamma")
        if "bias" in temp and "norm" in temp:
    ...
    for item in compare.items():
        ...
        # dd=params_dict[tk]
        # rr=params_dict[tk].numpy()
        pair['data'] = Tensor(params_dict[tk].numpy())
        # if pair['name'] == "blocks.0.mlp.fc1.weight":
        #     tt=pair['data']
        #     s=0
