...
def get_optimizer(optimizer_name, network, lrs):
    no_weight_decay_filter = "beta,bias"
    no_weight_decay_filter = [x for x in no_weight_decay_filter.split(",") if len(x) > 0]
    group_params = paramter_group(network, 0.05, no_weight_decay_filter, bool(0))

    if optimizer_name == 'adamw':
        opt = AdamW(group_params, lrs, 0.9, 0.999, loss_scale=1024)
    else:
